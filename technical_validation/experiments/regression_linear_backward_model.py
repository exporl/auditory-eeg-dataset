...
def evaluate_model(model, test_eeg, test_env):
    pred_env = np.matmul(test_eeg, model)
    return scipy.stats.pearsonr(pred_env[:, 0], test_env[:, 0])[0]

def permutation_test(model, eeg, env, tmin, tmax, numPermutations=100):
    pred_env = np.matmul(eeg, model)
    corrs = []
    for permutation_index in range(numPermutations):
        print(f'Permutation {permutation_index+1:03d}\r', end='')
        random_shift = np.random.randint(tmax-tmin, env.shape[0] - (tmax-tmin))
        temp_env = np.roll(env, random_shift, axis=0)
        # temp_eeg = temp_eeg[random_shift:, :]
        # temp_pred_eeg = pred_eeg[:-random_shift, :]

        corrs.append(scipy.stats.pearsonr(temp_env[:, 0], pred_env[:, 0])[0])
...
def crossval_over_recordings(all_data, tmin, tmax, ridge_param):
    ...
    for fold in range(len(all_data)):
        ...
        train_eegs = [time_lag_matrix(eeg, tmin, tmax) for eeg in train_eeg_folds]
        cxx = np.sum([np.matmul(x.T, x) for x in train_eegs], axis=0)
        cxy = np.sum([np.matmul(x.T, y[:-(tmax - tmin) + 1, :]) for x, y in zip(train_eegs, train_env_folds)], axis=0)
        ...
        if not isinstance(ridge_param, (int, float)):
            ...
            for lambd in ridge_param:
                ...
                score = evaluate_model(model,time_lag_matrix(test_eeg_fold, tmin, tmax), test_env_fold[:-(tmax - tmin) + 1, :])
...
def training_loop(subject, train_files, test_files, highpass, lowpass, tmin, tmax, ridge_param):
    ...
    train_eegs = [time_lag_matrix(x[0], tmin, tmax) for x in all_data]
    train_envs = [x[1] for x in all_data]
    cxx = np.sum([np.matmul(x.T, x) for x in train_eegs], axis=0)
    cxy = np.sum([np.matmul(x.T, y[:-(tmax-tmin)+1, :]) for x, y in zip(train_eegs, train_envs)], axis=0)
    ...
    for test_filenames, test_seg in test_generator:
        ...
        test_eeg = time_lag_matrix(test_eeg, tmin, tmax)
        # shorten eeg to match the length of the env
        test_env = test_env[:-(tmax-tmin)+1, :]
        # predict
        pearson_scores = evaluate_model(model, test_eeg, test_env)
...
if __name__ == "__main__":
    ...
    # beta (14 - 30)
    # broadband (0.5 - 32)
    parser = argparse.ArgumentParser(description='Train and evaluate a linear backward model. If no highpass or lowpass frequency is specified, the model will be trained and evaluated on the following frequency bands: delta (0.5 -4 ), theta (4 - 8), alpha (8 - 14), beta (14 - 30), broadband (0.5 - 32).')
    ...
    parser.add_argument('--tmin', type=float, default=-0.1, help="Minimum of the integration window (in s)")
    parser.add_argument('--tmax', type=float, default=0.4, help="Maximum of the integration window (in s)")
